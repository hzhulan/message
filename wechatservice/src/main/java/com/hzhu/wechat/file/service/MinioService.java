package com.hzhu.wechat.file.service;
...
public class MinioService {
    ...
    public MinioClient getInstance() {
        if (minioClient == null) {
            ...
            makeBucketIfNotExist(this.bucketName);
        }
        return minioClient;
    }
    ...
    public String upload(InputStream inputStream, String objectName) {
        try {
            ...
            return new StringBuilder().append(minIoHttpUrl).append(Constants.LINE_FLAG)
                    .append(bucketName).append(Constants.LINE_FLAG).append(path).toString();
    ...
    public List<String> listFiles(String prefix) {
        ...
        for (Iterator<Result<Item>> it = results.iterator(); it.hasNext();) {
            ...
            fileList.add(new StringBuilder().append(minIoHttpUrl).append(Constants.LINE_FLAG).append(bucketName).append(Constants.LINE_FLAG).append(fileName).toString());
        }
    ...
    public void deleteAll() {
        try {
            ListObjectsArgs params = ListObjectsArgs.builder().bucket(bucketName).build();
            ...
            RemoveObjectsArgs deleteParams = RemoveObjectsArgs.builder().bucket(bucketName).objects(deleteList).build();
